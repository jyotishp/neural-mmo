...
def vacant(tile):
   ...
   crystal = material.Crystal.index
   fragment= material.Fragment.index
   weeds   = material.Weeds.index
   herb    = material.Herb.index

   return matl in (grass, scrub, forest, crystal, fragment, weeds, herb) and not occupied
...
def forageDijkstra(config, ob, actions, food_max, water_max, cutoff=100):
   ...
   while not queue.empty():
      ...
      for nxt in adjacentPos(cur):
         ...
         if matl in (material.Lava.index, material.Water.index, material.Stone.index):
            continue
...
def aStar(config, ob, actions, rr, cc, cutoff=100):
   ...
   while not pq.empty():
      ...
      for nxt in adjacentPos(cur):
         ...
         if matl in (material.Lava.index, material.Stone.index):
            continue
